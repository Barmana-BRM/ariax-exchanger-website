...
final class SmsJobHandler implements JobHandlerInterface
{
    ...
    public function __invoke(Job $job): void
    {
        ...
        if ($recipient === '' || $message === '') {
            throw new RuntimeException('SMS job payload is invalid.');
        }
    ...
    private function appendJsonLine(array $entry): void
    {
        ...
        if ($encoded === false || file_put_contents($path, $encoded . PHP_EOL, FILE_APPEND | LOCK_EX) === false) {
            throw new RuntimeException('Unable to append SMS outbox entry.');
        }
    }
}
