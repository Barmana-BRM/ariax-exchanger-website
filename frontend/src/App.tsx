...
import { authApi, txApi, usersApi, ticketsApi, loadAppData, normalizeUser, assetUrl } from './services/api';
...
interface AppUser {
  id: string; name: string; avatarColor: string; role: "admin" | "user";
...
interface TeamMessage { id: string; senderId: string; senderName: string; message: string; timestamp: string; }
interface TeamTask { id: string; title: string; assignedTo: string; status: "todo" | "in_progress" | "done"; createdAt: string; category: "wallet" | "support" | "technical" | "liquidity"; }
...
//  INITIAL DATA
// ══════════════════════════════════════════════════════════════
const NATIONAL_DB: Record<string, string> = {
  "0012345678": "09121234567",
  "0019876543": "09351234567",
  "0045678901": "09011234567",
};

const INITIAL_USERS: AppUser[] = [
  {
    id: "admin1", name: "مدیر سیستم", avatarColor: "#10b981", role: "admin",
    username: "admin", password: "admin123",
    balances: { IRT: 0, BTC: 0, ETH: 0, USDT: 0, TRX: 0 },
    cryptoAddresses: { BTC: "", USDT: "", TRX: "" },
    cardNo: "", shibaNo: "",
    kycStatus: "verified",
    kycVerified: true,
    kycDetails: { fullName: "مدیر سیستم", nationalId: "0000000000", phone: "09000000000", timestamp: "1403/01/01" }
  },
  {
    id: "u1", name: "علی رضایی", avatarColor: "#3b82f6", role: "user",
    username: "user1", password: "user1234",
    balances: { IRT: 28500000, BTC: 0.0042, ETH: 0.35, USDT: 240, TRX: 1200 },
    cryptoAddresses: { BTC: "bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh", USDT: "TQn9Y2khEsLJW1ChVWFMSMeRDow5KcbLSE", TRX: "TXQ8v2LsQeEaFMG7Rx8vJ9Ep6sDtaVXnFR" },
    cardNo: "6037991234567890", shibaNo: "IR120570028080013447370101",
    kycStatus: "verified",
    kycVerified: true,
    kycDetails: { fullName: "علی رضایی", nationalId: "0012345678", phone: "09121234567", timestamp: "1403/02/15" }
  },
];

function makeHistory(base: number, count = 30) {
  const arr = [];
  let p = base;
  const now = Date.now();
  for (let i = count; i >= 0; i--) {
    ...
    arr.push({ time: `${d.getHours()}:00`, price: Math.round(p), volume: Math.round(Math.random() * 500 + 100) });
  }
  return arr;
}

const INITIAL_MARKET: MarketTicker[] = [
  { symbol: "BTC", name: "Bitcoin", faName: "بیت‌کوین", priceIRT: 3800000000, priceUSD: 67000, change24h: 2.4, volume24h: 28500, high24h: 3850000000, low24h: 3720000000, history: makeHistory(67000) },
  { symbol: "ETH", name: "Ethereum", faName: "اتریوم", priceIRT: 190000000, priceUSD: 3350, change24h: -1.2, volume24h: 14200, high24h: 195000000, low24h: 187000000, history: makeHistory(3350) },
  { symbol: "USDT", name: "Tether", faName: "تتر", priceIRT: 56000, priceUSD: 1.0, change24h: 0.1, volume24h: 95000, high24h: 56200, low24h: 55800, history: makeHistory(1) },
  { symbol: "TRX", name: "Tron", faName: "ترون", priceIRT: 680, priceUSD: 0.12, change24h: 3.1, volume24h: 8700, high24h: 700, low24h: 660, history: makeHistory(0.12) },
];

const INITIAL_TXS: Transaction[] = [
  { id: "t1", userId: "u1", userName: "علی رضایی", type: "deposit", asset: "IRT", amount: 10000000, fee: 50000, timestamp: "1403/12/01 10:22", status: "completed", destination: "بانک ملی" },
  { id: "t2", userId: "u1", userName: "علی رضایی", type: "trade", asset: "BTC", amount: 0.002, fee: 0.000004, timestamp: "1403/12/03 09:10", status: "completed", destination: "تبدیل به USDT" },
  { id: "t3", userId: "u1", userName: "علی رضایی", type: "withdraw", asset: "IRT", amount: 3000000, fee: 15000, timestamp: "1403/12/09 15:30", status: "pending", destination: "بانک صادرات" },
  { id: "t4", userId: "u1", userName: "علی رضایی", type: "deposit", asset: "IRT", amount: 20000000, fee: 100000, timestamp: "1403/11/20 14:40", status: "completed", destination: "بانک ملی" },
];

const INITIAL_MESSAGES: TeamMessage[] = [
  { id: "m1", senderId: "admin1", senderName: "مدیر سیستم", message: "سلام تیم! لطفاً درخواست‌های برداشت امروز رو بررسی کنید.", timestamp: "1403/12/10 09:00" },
  { id: "m2", senderId: "u1", senderName: "علی رضایی", message: "چشم، الان بررسی می‌کنم.", timestamp: "1403/12/10 09:05" },
];

const INITIAL_TASKS: TeamTask[] = [
  { id: "tk1", title: "بررسی درخواست‌های برداشت", assignedTo: "u1", status: "in_progress", createdAt: "1403/12/10", category: "wallet" },
  { id: "tk2", title: "به‌روزرسانی نرخ کارمزد", assignedTo: "admin1", status: "todo", createdAt: "1403/12/10", category: "technical" },
];
  ...
  async function doRegister() {
    ...
    try {
      ...
      createdUser = normalizeUser(result.user ?? {
        id: "u" + Date.now(), name: fullName || nationalId, avatarColor: "#8b5cf6", role: "user",
        username, password,
        balances: { IRT: 0, BTC: 0, ETH: 0, USDT: 0, TRX: 0 },
        cryptoAddresses: { BTC: "", USDT: "", TRX: "" },
        cardNo: "", shibaNo: "",
        kycStatus: "pending",
        kycDetails: { fullName: fullName || nationalId, nationalId, phone, email: email.trim() || undefined, timestamp: nowStr() },
      });
    } catch (err: unknown) {
  ...
  async function attempt() {
    ...
    try {
      ...
      if (isAdmin && user.role !== "admin") {
        authApi.logout();
        setErr("این صفحه فقط برای ورود مدیر سیستم است");
        return;
      }
...
function MarketPage({ market }: { market: MarketTicker[] }) {
  ...
  const coin = market.find(m => m.symbol === sel)!;
  return (
...
// ══════════════════════════════════════════════════════════════
function DepositWithdrawPage({ user, onAddTx }: { user: AppUser; onAddTx: (t: Transaction) => void }) {
  ...
  function submit() {
    ...
    onAddTx(tx);
    setAmount(""); setDest(""); setHomeAddress(""); setPostalCode(""); setDone(true);
...
function UserShell({ user, users, txs, market, tickets, onAddTx, onUpdateUser, onRefreshTickets, onLogout }: {
  ...
  tickets: Ticket[];
  onAddTx: (t: Transaction) => void; onUpdateUser: (u: AppUser) => void;
...
function AdminShell({ admin, users, txs, market, tickets, onUpdateUser, onUpdateTx, onLogout, onAddTx, onRefreshTickets }: {
  ...
  onAddTx: (t: Transaction) => void; onRefreshTickets: () => void | Promise<void>;
}) {
    ...
    <div className="flex min-h-screen bg-slate-900" dir="rtl" style={{ fontFamily: "Vazirmatn,sans-serif" }}>
      <aside className="w-52 bg-slate-800/80 border-l border-slate-700/50 flex flex-col shrink-0">
        ...
        <div className="p-3 border-b border-slate-700/50 flex items-center gap-2">
          ...
          <div><div className="text-white text-xs font-semibold">{admin.name}</div><div className="text-emerald-400 text-[10px]">ادمین</div></div>
        </div>
...
export default function App() {
  ...
  const [market, setMarket] = useState<MarketTicker[]>(INITIAL_MARKET);
  useEffect(() => {
    const interval = setInterval(() => {
      setMarket(prev => prev.map(m => {
        const delta = (Math.random() - 0.49) * 0.008;
        const newPrice = Math.round(m.priceIRT * (1 + delta));
        const newUSD = +(m.priceUSD * (1 + delta)).toFixed(m.priceUSD > 10 ? 2 : 4);
        const newChange = +(m.change24h + delta * 10).toFixed(2);
        const newHist = [...m.history.slice(1), { time: `${new Date().getHours()}:${String(new Date().getMinutes()).padStart(2, "0")}`, price: newPrice, volume: Math.round(Math.random() * 300 + 100) }];
        return { ...m, priceIRT: newPrice, priceUSD: newUSD, change24h: newChange, history: newHist };
      }));
    }, 4000);
  ...
  useEffect(() => {
    ...
    if (user.role === "admin" && !isAdminRoute) navigate("/admin");
    else if (user.role === "user" && isAdminRoute) navigate("/");
    setCurrentUser(user);
    void loadAppData(user.role).then(data => {
      setTxs(data.transactions);
      setTickets(data.tickets);
      if (user.role === "admin") setUsers(data.users);
  ...
  async function handleLogin(u: AppUser) {
    ...
    try {
      const data = await loadAppData(user.role);
      setTxs(data.transactions);
      setTickets(data.tickets);
      if (user.role === "admin") setUsers(data.users);
    } catch {
  ...
  function handleRegister(u: any) { setUsers(prev => [...prev, normalizeUser(u)]); navigate("/"); }
